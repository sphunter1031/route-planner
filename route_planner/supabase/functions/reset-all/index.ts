...
const VERSION = "reset-all v5.2 (update-only) 2026-02-01 15:50 KST";
const TABLE = "daily_plan_items";

type Body = { plan_date: string };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // UPDATE-only로: seq 재부여 + locked/manual 해제
    for (let i = 0; i < sorted.length; i++) {
      ...
      const { error: upErr } = await supabase
        .from(TABLE)
        .update({ seq: newSeq, locked: false, is_manual: false })
        .eq("id", r.id);
